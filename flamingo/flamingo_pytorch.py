...
class MaskedCrossAttention(nn.Module):
    ...
    def forward(self,
        ...
        if exists(media_locations):
            ...
            text_to_media_mask = mask_op(rearrange_many(text_time, 'b i -> b 1 i 1'),
        ...
        if exists(media_locations) and self.only_attend_immediate_media:
            text_without_media_task = text_time == 0
            text_without_media_task = rearrange(text_without_media_task, 'b i -> b 1 i 1')
            attn.masked_fill_(text_without_media_task, 0.)

        out = einsum('... i j, ... j d -> ... i d', attn, v)
        out = rearrange(out, 'b h n d -> b t n (h d)', h = h)
        return self.to_out(out)
...
class GatedCrossAttentionBlock(nn.Module):
    ...
    def forward(self,
                ...
                media,
                media_locations = None):
        x = self.attn(x, media, media_locations) * self.attn_gate.tanh() + x
        x = self.ff(x)
